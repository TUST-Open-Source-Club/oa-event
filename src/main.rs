...
use event_service::migration::Migrator;
use event_service::state::{AppState, SharedState};
use event_service::{build_router, db};
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing::info!(bind = %config.bind_addr, "event 服务启动中");

    let database = db::connect_with_schema(&config.database_url, "event")
        .await
        .context("连接数据库失败")?;
